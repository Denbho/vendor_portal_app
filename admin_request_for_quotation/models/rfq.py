...
class AdminVendorRFQ(models.Model):
    ...
    def btn_accept(self):
          if self.rfq_id and self.rfq_id.close_date < fields.Date.today():
...
class AdminRequestForQuotationLine(models.Model):
    ...
    delivery_lead_time = fields.Integer(string="Delivery Lead Time", help="In Days", copy=False)
    gross_total = fields.Float(string="Gross Total", store=True, compute="_get_total")
    warranty = fields.Text(string="Warranty", copy=False)
    terms = fields.Text(string="Payment Terms", copy=False)
    validity_from = fields.Date(string="Valid From", copy=False)
    validity_to = fields.Date(string="Valid To", copy=False)
    minimum_order_qty = fields.Float(string="Minimum Order Quantity")
    ...
    def reverse_selected_vendor(self):
        selected_vendors = self.env['admin.request.for.quotation.line.vendor'].sudo().search([('rfq_line_id','=',self.id),('selected','=','selected')])
        for line in selected_vendors:
            line.selected = False
            vendor_pricelist = self.env['product.supplierinfo'].sudo().search([
                                        ('product_id', '=', line.product_id.id), ('name', '=', line.partner_id.id),
                                        ('date_start', '=', line.validity_from),('date_end', '=', line.validity_to),
                                        ('min_qty', '=', line.minimum_order_qty),('delay', '=', line.delivery_lead_time),
                                        ('price', '=', line.price)], limit=1)
            if vendor_pricelist:
                vendor_pricelist.unlink()
        self.assigned_vendor_ids = False
...
class AdminRequestForQuotation(models.Model):
    ...
    open_date = fields.Date(string="Opening Date", copy=False, readonly=True,
                             states={'draft': [('readonly', False)]})
    close_date = fields.Date(string="Closing Date", copy=True,
                             help="Last date of quotation for the vendor")
    rfq_line_ids = fields.One2many('admin.request.for.quotation.line', 'rfq_id', copy=True)
    ...
    rtd_reason_id = fields.Many2one('admin.cancel.and.halt.reason', string='Reset to Draft Reason', tracking=True, copy=False)
    rtd_description = fields.Text(string='Reset to Draft Reasons Description', tracking=True, copy=False)
    ...
    def send_email_to_vendors(self):
        ...
        rfq_mails = self.env['admin.vendor.rfq'].sudo().search([('rfq_id','=',self.ids[0]),('state','not in',['canceled','declined','no_response'])])
        partner_ids = []
        for line in rfq_mails:
            partner_ids.append(line.partner_id.id)
        ctx = dict(self.env.context or {})
        ctx.update({
            'custom_layout': "mail.mail_notification_paynow",
            'default_model': 'admin.request.for.quotation',
    ...
    def message_post(self, **kwargs):
        if self.env.context.get('send_email_to_vendors'):
            ...
            rfq_mails = self.env['admin.vendor.rfq'].sudo().search([('rfq_id','=',self.ids[0]),('partner_id','in',partner_ids)])
            for line in rfq_mails:
                kwargs['partner_ids'] = [line.partner_id.id]
                line.message_post(**kwargs)
            kwargs['partner_ids'] = []
        return super(AdminRequestForQuotation, self).message_post(**kwargs)
    ...
    def _date_validation(self):
        for rec in self:
            if rec.open_date:
                if rec.open_date < date.today() and rec.state == 'draft':
    ...
    def set_rfq_done(self):
        ...
        for r in vendor_rfq.search([('rfq_id', '=', self.id)]):
            ...
            if r.state not in ['declined', 'no_response']:
                r.state = 'done'
    ...
    def create_vendor_rfq_line(self):
        ...
        for ven in set(vendors):
            ...
            data = data.with_context(default_message_type='comment', default_recipient_ids=[ven],
                                    default_notification=True, default_partner_ids=[ven])
            if subtype_id:
                data = data.with_context(default_subtype_id=subtype_id.id)
            template_id = data.env.ref('admin_request_for_quotation.email_template_request_for_quotation').id
    ...
    def _opening_date_send_invitation_email(self):
        records = self.sudo().search([('state', '=', 'approved'),('open_date', '<=', fields.Date.today())])
        for rec in records:
            rec.create_vendor_rfq_line()

    @api.model
    def _update_no_response_vendor(self):
        records = self.sudo().search([('state', '=', 'sent'),('close_date', '<=', fields.Date.today())])
        for rec in records:
            for line in self.env['admin.vendor.rfq'].sudo().search([('rfq_id', '=', rec.id), ('state', '=', 'waiting_for_acceptance')]):
